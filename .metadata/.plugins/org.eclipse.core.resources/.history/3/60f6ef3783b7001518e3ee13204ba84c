...
public class GamePanel extends SurfaceView implements SurfaceHolder.Callback, Runnable {
	...
	public void init() {
		...
		bmCraft = Bitmap.createScaledBitmap(temp, craft.getCraftWidth(), craft.getCraftHeight(), true);

		temp = BitmapFactory.decodeResource(getResources(), R.drawable.crashed_left);
		bmCraftFallLeft = Bitmap.createScaledBitmap(temp, craft.getCraftHeight(), craft.getCraftWidth(), true);

		temp = BitmapFactory.decodeResource(getResources(), R.drawable.crashed_right);
		bmCraftFallRight = Bitmap.createScaledBitmap(temp, craft.getCraftHeight(), craft.getCraftWidth(), true);
	...
	private void doDraw(Canvas canvas) {
		...
		if (gameLoop.getState() == GameLoop.STATE_READY) { 
			canvas.drawBitmap(bmCraft, (screenWidth - Craft.getCraftWidth()) / 2, screenHeight / 2, null);
			return;
		}
		...
		if (gameLoop.getState() == GameLoop.STATE_CRASHED) {// if the craft crashed
			...
			return;
		} else if (gameLoop.getState() == GameLoop.STATE_FALL_LEFT) { //crashed when the craft landed failed on the left side.
			canvas.drawBitmap(bmCraftFallLeft, craft.getX(), craft.getCenterY(), null);
			canvas.drawBitmap(bmLose, (screenWidth - bmWon.getWidth())/2, screenHeight / 2, null);
			return;
		} else if (gameLoop.getState() == GameLoop.STATE_FALL_RIGHT) {//crashed when the craft landed failed on the right side.
			canvas.drawBitmap(bmCraftFallRight, craft.getCenterX(), craft.getCenterY(), null);
			canvas.drawBitmap(bmLose, (screenWidth - bmWon.getWidth())/2, screenHeight / 2, null);
			return;
		} else if (gameLoop.getState() == GameLoop.STATE_WON) {// landed successfully
			canvas.drawBitmap(bmCraft, matrix, null);
