...
public class MarsLanderView extends SurfaceView implements SurfaceHolder.Callback, Runnable {
	...
	public void init() {
		...
		bmCraft = Bitmap.createScaledBitmap(temp, Craft.CRAFT_WIDTH, Craft.CRAFT_HEIGHT, true);

		temp = BitmapFactory.decodeResource(getResources(), R.drawable.crashed_left);
		bmCraftFallLeft = Bitmap.createScaledBitmap(temp, Craft.CRAFT_HEIGHT, Craft.CRAFT_WIDTH, true);

		temp = BitmapFactory.decodeResource(getResources(), R.drawable.crashed_right);
		bmCraftFallRight = Bitmap.createScaledBitmap(temp, Craft.CRAFT_HEIGHT, Craft.CRAFT_WIDTH, true);
	...
	private void doDraw(Canvas canvas) {
		...
		if (scene.getState() == MarsLanderScene.STATE_READY) { // show the first view of this app, draw the craft at the center on screen.
			canvas.drawBitmap(bmCraft, (screenWidth - Craft.CRAFT_WIDTH) / 2, screenHeight / 2, null);
			return;
		}
