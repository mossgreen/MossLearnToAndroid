package com.victor.marslander.model;

import java.util.Random;
import java.util.Vector;

import com.victor.marslander.Mars;
...
public class MarsLanderScene {
	...
	private void init() {
		// create craft
		int posX = Craft.CRAFT_WIDTH * 2 + rdm.nextInt(screenWidth - Craft.CRAFT_WIDTH * 4);
		craft = new Craft(posX, 0, GRAVITY, PIXELMETERRATIO);

		mars = new Mars(0.3f, screenWidth, screenHeight, (int) (Craft.CRAFT_WIDTH * 1.3f), Craft.CRAFT_WIDTH * 2, 300);
		//
	...
	public void update() {
		...
		if (craft.getCenterX() < 0) {
			craft.setX(screenWidth - Craft.CRAFT_WIDTH / 2);
		} else if (craft.getCenterX() > screenWidth) {
			craft.setX(-Craft.CRAFT_WIDTH / 2);
		}
		...
		if (!p.isEmpty()) {
			if (!isFlatGround(craft.getX(), Craft.CRAFT_WIDTH)) {
				state = STATE_CRASHED;
			} else {
				if (craft.getAngle() > 0) {
